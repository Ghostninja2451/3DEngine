...
namespace henry
{
	...
	public:
		...
		void Add(const std::string& name, std::shared_ptr<henry::Resource> resource);
	...
	inline void ResourceSystem::Add(const std::string& name, std::shared_ptr<henry::Resource> resource)
	{
		resources[string_tolower(name)] = resource;
	}
